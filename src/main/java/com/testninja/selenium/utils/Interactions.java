    ...
    public void waitForAngular() {
        try {
            ...
            ((JavascriptExecutor)driver).executeAsyncScript("var callback = arguments[arguments.length - 1];\n" +
                    "\n" +
                    "angular.element(document.querySelector(\"body\")).injector().get(\"$browser\").notifyWhenNoOutstandingRequests(callback)");
        } catch (ScriptTimeoutException e) {
//            System.out.println("Script time out exception occurred");
        }
    }
